...
## medusa - Brute force SSH login for the `admin` user
```
medusa -h 192.168.1.100 -u admin -P passwords.txt -M ssh
```
## medusa - Brute force FTP with multiple usernames and passwords using 5 parallel threads.
```
medusa -h 192.168.1.100 -U users.txt -P passwords.txt -M ftp -t 5
```

## medusa - Brute force RDP login
```
medusa -h 192.168.1.100 -u admin -P passwords.txt -M rdp
```
## medusa - Brute force HTTP Basic Authentication
```
medusa -h www.example.com -U users.txt -P passwords.txt -M http -m GET
```

## medusa - Stop after the first valid SSH login is found
```
